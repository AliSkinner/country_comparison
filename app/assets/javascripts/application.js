  ...
  var pointer = screen.pointer.init({
    ...
    up: function () {
      ...
      if (rec) {
        ...
        PHY2D.rectangle(Math.min(rec.x0, rec.x1) + w / 2, Math.min(rec.y0, rec.y1) + h / 2, w, h, 1, 0);
        rec = null;
      }
    }
  });
  
  // small vectors 2D lib (does not create/return new objects at runtime)
  ...
  var PHY2D = function (ctx, pointer, Vector, Matrix) {
    ...
    Polygon.prototype = {
      ...
      contact: function (that) {
        ...
        if (!this.featurePairJudgement(that, 2)) return;
        
        // faces of B, vertices of A
        if (!that.featurePairJudgement(this, 1)) return;
